...
```{r echo=FALSE}
listY <- list("Potential_INEXUS_in_dollar","duration_door_to_door", "Realized_INEXUS_in_dollar", "Realized_INEXUS")
print(listY)
```
...
listHet       <- list("ownCarYNLabel", "l_inc_HiLo10","mode_choice_actual_6")
...
```{r}
write_excel_csv(description_df, file ="ScenarioTitlesColorsEtc.csv")
```
...
# facet wrap LEVER
  plottttt <- plotttaaaa0 +
    ...
    if(saveGraphsTF == TRUE) {
         ggsave(str_glue("figures/{Sys.Date()}_{nm}_{yvarShortName}_{het}_{category_short}_wrap-{lever_short}_{year}.png"))
         ggsave(str_glue("figures/{Sys.Date()}_{nm}_{yvarShortName}_{het}_{category_short}_wrap-{lever_short}_{year}.pdf"))
         # ggsave(str_glue("figures/{Sys.Date()}_a.png"))
         # ggsave(str_glue("figures/{Sys.Date()}_a.pdf"))
         }
# facet wrap HET    
  plottttt <- plotttaaaa0 +
    geom_density(
    ...
    facet_wrap(facets = as_factor(df_stacked_from_list$ownCarYNLabel) , ncol = 3) 
    print(plottttt)
    if(saveGraphsTF == TRUE) {
         # ggsave(str_glue("figures/{Sys.Date()}_b.png"))
         # ggsave(str_glue("figures/{Sys.Date()}_b.pdf"))
         ggsave(str_glue("figures/{Sys.Date()}_{nm}_{yvarShortName}_wrap-{het}_{category_short}_{lever_short}_{year}.png"))
         ggsave(str_glue("figures/{Sys.Date()}_{nm}_{yvarShortName}_wrap-{het}_{category_short}_{lever_short}_{year}.pdf"))
    }
return(plottttt)
}
