...
description_df <- description_df |>
  mutate(
    ...
    yaxishigh =
      case_when(
        variable == "logsum_trip_mode_AS_trips" ~
          median_hilow(df_stacked_from_list$logsum_trip_mode_AS_trips, conf.int =
                         .95)[["ymax"]],
...
fx_SUMMARY_STATS <- function(df) {
  ...
  sssumm <- df %>%
    ...
    summarise(
      ...
      avY = mean(.data[["logsum_trip_mode_AS_trips"]]), 
      n=n()
    )
  print(sssumm)
  sssummQuantiles <- df %>%
    ...
    summarise(
      ...
      qsTIV=quantile(.data[["logsum_trip_mode_AS_trips"]], na.rm = TRUE),
      n=n()
    )
