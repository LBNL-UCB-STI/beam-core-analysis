...
df_lorenz <- df_lorenz |> 
    group_by(lever_position_fleetsize,lever_position_price)|> 
      arrange(incomeRank0to20_avg, .by_group = TRUE) |> 
        rename(income = income_in_thousands_avg) |> 
        mutate(incomeCum = cumsum(income)) |> 
        mutate(incomePctOfTotalInc = income / max(incomeCum)) |> 
        mutate(incomePctOfTotalIncCum =  incomeCum / max(incomeCum)) |> 
        rename(incRank = incomeRank0to20_avg) |> 
        ...
        mutate(DoorToDoorTimePctOfTotalTimeCum = DoorToDoorTimeCum/max(DoorToDoorTimeCum)) |> 
...
df_lorenz |> group_by(lever_position_fleetsize, lever_position_price) |> 
  summarise(n = n())
```
...
p1 <- ggplot(data = df_lorenz |> 
               ...
               filter(lever_position_fleetsize == 0.25)  )
p1 <- p1 + aes(alpha = lever_position_price,
               color = lever_position_fleetsize,
...
p3
p6 <- p1 + aes(x =  incPercentRank)                         # incPercentRank income20levels income_in_thousands_avg incPct    incCumPct        incCumPct
p6 <- p6 + geom_line(aes(y = DoorToDoorTimePctOfTotalTime)) # duration_door_to_door_avg durationDoorDoorAvgPctCum
p6 <- p6 + geom_line(aes(y = DoorToDoorTimeAveragePctOfTotalTime), alpha = .5, linetype = 2)                     # durationDoorDoorAvgAvg    durationDoorDoorAvgAvgCumPct
p6
p4 <- p1 + aes(x =   incPercentRank)                         # incPercentRank income20levels income_in_thousands_avg incPct    incCumPct        incCumPct
p4 <- p4 + geom_line(aes(y = DoorToDoorTimePctOfTotalTimeCum)) # duration_door_to_door_avg durationDoorDoorAvgPctCum
...
```{r Actual Gini}
pGini <- p1 + 
          aes(x =  incPercentRank)  +
          geom_line(aes(y=incomePctOfTotalIncCum)) +
