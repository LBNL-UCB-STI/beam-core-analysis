...
# Start with whatever Stacked big dataset you are using
...
```{r graph dataset}
df_graph <- df_lorenz |>  filter(lever_position_price ==1)
...
p2 <- p1 +  geom_line(mapping =  aes(x = incCumPct, y = .data[[yvar]],
                                     ...
                                     group = lever_position_fleetsize)) 
p2
priceLevelVector <- as.numeric(levels(as_factor(df_graph$lever_position_fleetsize)))
