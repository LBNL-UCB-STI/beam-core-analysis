...
descr <- descr |> relocate("type", "title") |> filter(V1 != FALSE)
```
...
# get names etc from the description file    
  yvarTitle <- filter(descr,variable == y_var)[["title"]]
  yvarTitleShort <- filter(descr,variable == y_var)[["title_short"]]
  hetTitle <- filter(descr,variable == het)[["title"]]
  hetvarTitleShort <- filter(descr,variable == het)[["title_short"]]
...
print("Now  plot  heterogeneity,  separate graphs into different levers")
  ...
  print(plottttta1)
    # if(saveGraphsTF == TRUE) {
    #      ggsave(str_glue("figures/{Sys.Date()}_{categoryTitleShort}_{leverTitleShort}___Y-{yvarShortName}_H-{hetShortName}_Faceted-levers.png"))
    #      ggsave(str_glue("figures/{Sys.Date()}_{categoryTitleShort}_{leverTitleShort}___Y-{yvarShortName}_H-{hetShortName}_Faceted-levers.pdf"))
    #       }
