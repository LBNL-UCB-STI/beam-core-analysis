...
df_temp <- df_temp |>
  mutate(lever_position =
           case_when(lever_position == "base" ~ 1,
                     TRUE ~ lever_position))
df_temp <- df_temp |>
  mutate(lever_positionFactor =as_factor(lever_position)) 
...
write_rds(df_temp, file =paste0(data_dir_on_this_machine,
                   glue("{placeFilename}_{year}_stacked_{categoryTitleShort}_{leverTitleShort}_6a.rds")))
```
...
<!--                    glue("{placeFilename}_{year}_stacked_{categoryTitleShort}_{leverTitleShort}_3.rds"))) -->
...
df_temp <- df_temp |> 
  ...
  mutate(mode_planned_at_baseline = first(mode_planned_5[lever_position==1]))
ungroup(df_temp)
