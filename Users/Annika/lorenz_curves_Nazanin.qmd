...
df_lorenz <- read.csv(file = "C:/Users/nazan/Documents/GitHub/beam-core-analysis/Users/Annika/lorenz.csv")
#df_lorenz <- readr::read_rds(file = paste0(data_dir_on_this_machine,
                   #glue("{placeTitleShort}_{year}_",
                        #"{categoryTitleShort}_{leverTitleShort}_",
                        #"lorenzWithCums",
...
p1 <- ggplot(data = df_lorenz |> filter(lever_position > -1) |> 
...
p8 <- p8 + geom_line(aes(y = logsum_relative_to_baseline),size=1.5) # duration_door_to_door_avg durationDoorDoorAvgPctCum
# Formatting
p8 <- p8 +
      ylab("Potential INEXUS relative to Baseline") +
      xlab("Income Rank (Percentiles of Income)") +
      labs(color = "Ridehail Price")+
...
p9 <- p8 +
    scale_color_continuous(low="orange", high="maroon")
  
  p9

ggsave(plot = p9, file = "lorenz_new_orange.svg", width=10, height=8)
