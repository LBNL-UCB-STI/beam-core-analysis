...
if (rewrite==1) {
    if (ignoreAWS==FALSE) {awsDF <- get_bucket_df("beam-core-act", 
                           prefix = "deepDive/CleanData/SanFrancisco/Stacked") } # dataframe with all of the AWS files
...
p8 <- p8 + geom_line(aes(y = Pot_BetterWorse),size=1.5) # duration_door_to_door_avg durationDoorDoorAvgPctCum
# Formatting
p8 <- p8 +
...
p9 <- p8 +
      scale_color_continuous(high = "#470304", low = "#FF7815")
  

  p9

ggsave(plot = p9, file = "lorenz_old_orange.svg", width=10, height=8)
